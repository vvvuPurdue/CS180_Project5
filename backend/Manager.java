 ...
 * Manager
 *
 * Acts as a "database", storing all accounts and manages retrieval of specific
 * accounts The "model" of the entire system
 *
...
public class Manager {
    ...
    public int createAccount(String username, String password, String email, String phoneNumber, String bio,
            String interests) {
        // check to make sure that username doesn't already exist in the list/database
        // already
        int i = findUser(username);
        if (i == -1) {
    ...
    public int deleteAccount(String username, String password) {
        // check to make sure that user exists in our list/database, and password is
        // correct
        int i = findUser(username);
        if (i != -1) {
